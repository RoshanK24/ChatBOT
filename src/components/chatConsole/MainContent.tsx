...
const MainContent: React.FC<MainContentProps> = ({chatId, setChatId, allMessage, setAllMessage, isDark, setIsDark}) => {
  
  return (
    <div className='w-full h-screen bg-[#F6F5F2] dark:bg-[#282828] flex flex-col dark:text-white relative'>
        <ScrollArea className="grow flex flex-col relative">
            <Navbar isDark={isDark} setIsDark={setIsDark}/>
            <div className="min-h-full flex justify-center items-center ">
              <div className='w-3/5 h-full'>
              {chatId && allMessage.length > 0 ? (
        ...
        <div className="dark:bg-[#282828] bg-[#F6F5F2] flex justify-center items-center absolute z-20 bottom-0 w-full">
            <SearchBar allMessage={allMessage} setAllMessage={setAllMessage} chatId={chatId} setChatId={setChatId}/>
        </div> 
    </div>
  )
}
