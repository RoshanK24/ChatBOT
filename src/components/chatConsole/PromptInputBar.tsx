...
const PromptInputBar: React.FC<SearchBarProps> = ({
  allMessage,
  setAllMessage,
  chatId,
  setChatId,
  ...
  const handleSendMessage = async () => {
    ...
    const updatedMessages = [
      ...allMessage,
      { userText: userInput, chatBotText: "" },
    ];
    ...
    if (!chatId)
      allChat === null
        ? setAllChat([{ id: generatedchatId, chat_text: userInput }])
        : setAllChat([
            { id: generatedchatId, chat_text: userInput },
            ...allChat,
          ]);
    ...
    let text: string;
    try {
      text = await generateText(userMessage);
    } catch (error) {
      text = "I'm sorry, i can't generate the response to this quary.";
    }

    const finalMessages = [
      ...previousAllMessage,
      { userText: userInput, chatBotText: text },
    ];
  ...
  return (
    <div className="w-3/5 relative">
      <Input
        className="border-gray-500 mb-4 min-h-12 max-h-40 text-wrap shadow-sm dark:text-white"
      ...
      />
      <span
        onClick={isInputEmpty ? undefined : handleSendMessage}
        className={`absolute right-3 bottom-6 rounded-lg p-1 ${
          isInputEmpty
